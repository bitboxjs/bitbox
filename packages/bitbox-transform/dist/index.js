/*!
 * bitbox-transform v1.2.17 (Fri Jul 29 2016 11:27:09 GMT+0300 (EEST))
 * (c) 2016 Sergiu Toderascu <sergiu.toderascu@gmail.com> (http://bitboxjs.com)
 * MIT License.
 */
(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory() :
	typeof define === 'function' && define.amd ? define('bitbox-transform', factory) :
	(global.bitbox = global.bitbox || {}, global.bitbox.transform = factory());
				...
				for (var ei in node.attrs) {
					...
					if (prop) {
						if (prop.rel && prop.rel === 'def') {
							...
							outerexpr += '\n\t\t\t\t\t\t' + _export2 + newbox[0] + ' function ' + toCamel(node.name) + _args + ' {\n                            const ' + config.h + ' = arguments[1];\n                            ';
				...
				if (node.name === 'mod') {
					...
					if (node.content === -1) {
						...
						if (meta.local[name + '__s']) {} else {
							...
							var hasContent = node.content.trim().length;
							...
							if (_isnew || isnative) bodyornode = '' + _treectx[0] + config.h + '(\'' + node.name + '\'' + (_a ? ',' : '') + _a + (hasContent ? ',' : '') + treewrap[0];else if (_bt) bodyornode = '' + _treectx[0] + config.h + '(' + _bxname + (_a ? ',' : '') + _a + (hasContent ? ',' : '') + treewrap[0];else bodyornode = '' + _treectx[0] + _bxname + '(' + _a + (_a && hasContent ? ',' : '') + treewrap[0];
	...
	function transform$1(source, options) {
	    ...
	    result.code = result.code.replace(/export(\w)/g, "export $1").replace(/export\sdefault(\w)/g, "export default $1").replace(/return(\w)/g, "return $1");
	    return result.code;
	}
	...
	transform.load = function load() {
		var nodes = document.querySelectorAll('script[type="bitbox"]');
		if (nodes) {
			console.warn('loading ' + nodes.length + ' scripts');
			Array.prototype.map.call(nodes, function (source) {
				if (!source.bitboxLoaded) {
					source.bitboxLoaded = Date.now();
					var code = transform(source.textContent);
