const colors = {
	...
	random: (hex = false, i = 0)=> {
		const keys = Object.keys(colors)
		const item = colors.index[Math.floor(Math.random() * colors.index.length)]
...
export function hexToRGBA(color, alpha = 1) {
	...
	return 'rgba(' + values.r + ',' +
		values.g + ',' +
		values.b + ',' + alpha + ')';
}

function getColor(name, index = 500, alpha) {
	if (name === 'black' || name === 'white') index = ''
	return colors[name+index] ? hexToRGBA(colors[name+index], alpha) : 'inherit'
}

function getRandomColor(index = 500, alpha) {
	const item = colors.index[Math.floor(Math.random() * (colors.index.length))]
	return getColor(item, index, alpha)
}

function color(name, index = 500, alpha = 1) {
    if (index <= 1) {
        alpha = index
        index = 500
    }
